...
printCommand(cmd);
fetch('/api/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'command',data:cmd})}).catch(function(err){printOutput('Failed: '+err.message,'error')})}
...
return}
if(mainCmd==='devices'){
fetch('/api/command?client=web').then(function(res){return res.json()}).then(function(data){
...
sendCommand(cmd)}
function pollServer(){
fetch('/api/command?since='+lastId+'&client=web').then(function(res){return res.json()}).then(function(data){
